...
<div class="auction-item">
    <h3 class="auction-item__title">
        <a href="<?= Yii::$app->urlManager->createUrl(['/good', 'id' => $model->id]) ?>"><?= $model->name ?> (Лот
            №<?= $model->id ?>)</a>
    </h3>

    <div class="auction-item__img">
        <a href="<?= Yii::$app->urlManager->createUrl(['/good', 'id' => $model->id]) ?>"
           class="auction-item__img__link">
    ...
    <div class="auction-item__status">
        ...
        <?php else: ?>
            <span class="auction-item__starting"><img src="/assets_b/img/icon/sales.png"
                                                      alt="">Аукцион завершен - <?= $model->win_bid_id ? '<span class="auction-item__sales">ПРОДАНО</span>' : 'лот не продан' ?></span>
        <?php endif ?>
    </div>
        ...
        <ul class="action">
            <?php if (!Yii::$app->user->isGuest): ?>
                <li class="to-favorite" <?= ($cart->hasPositionFavorite($model->getId())) ? '' : 'style="opacity: 0.5"' ?>>
                    <a href="#" data-good-id="<?= $model->id ?>"
                       data-action="<?= ($cart->hasPositionFavorite($model->getId())) ? GoodFavorite::ACTION_REMOVE : GoodFavorite::ACTION_ADD ?>"><img
                                src="/assets_b/img/icon/item-star.svg" alt=""></a>
                </li>
            <?php endif ?>
